...
Grafiek <-
  ggplot(SelectieModel,
         aes_(x = ~Omtrek, y = ~HOOGTE, colour = ~TeControlerenAfwijking)) +
  scale_colour_manual(values = alpha(c("red", "black"))) +
  geom_jitter(width = 0.03, height = 0) +
  geom_line(aes_(y = ~H_D_finaal, colour = ~CurveSlecht, linetype = "Domein"),
            size = 1) +
  geom_point(aes_(y = ~H_D_finaal, colour = ~CurveSlecht),
...
if (has_name(SelectieModel, "H_VL_finaal")) {
  Grafiek <- Grafiek +
    geom_line(aes_(y = ~H_VL_finaal, linetype = "Vlaams"),
              colour = "black", size = 1)
}
