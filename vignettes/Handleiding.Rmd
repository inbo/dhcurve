...
Bij een geschatte hoogte die lager is dan 2,5 m, zal deze schatting vervangen worden door de vooropgestelde minimumwaarde van 2,5 m.
...
Als de hoogteschatting lager is dan 2,5 m, zal deze vervangen worden door een waarde van 2,5 m.
...
Bij voldoende gegevens in hogere omtrekklassen kan een model uitgebreid worden.
Dit wil zeggen dat het eerder gefitte model toegepast wordt op hogere omtrekklassen (`> Q95k`), waarna het model bij deze hogere omtrekklassen gevalideerd wordt op basis van de metingen.
...
De berekeningswijze van de meeste info uit de tabel die met de functie `resultaat()` gegenereerd wordt, is eerder uitgelegd.  De parameters `A`, `B` en `C` worden berekend door de in het modeltype aanwezige Vlaamse en domeinspecifieke parameters op te tellen.  Dit betekent:
