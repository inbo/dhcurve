...
__RMSE:__  De _**R**oot **M**ean **S**quare **E**rror_ is een waarde in meter die aangeeft hoe groot de spreiding van de metingen is t.o.v. het model en waar in feite ook uit afgeleid kan worden hoe goed het model de metingen benadert.  Gelijkaardig aan de _standaard error_ kan een betrouwbaarheidsinterval berekend worden voor de geschatte hoogten (op basis van het model) door 2 maal de waarde van de RMSE op te tellen en af te trekken van de geschatte hoogte.
...
De functie `outputIVANHO` genereert op basis van de berekende (gefitte) modellen een tabel met per boomsoort-domeincombinatie en per diameterklasse een geschatte hoogte, om te gebruiken in `IVANHO`.
...
- `nBomenTotOmtrek05`: aantal metingen > 0.5 m (dus het aantal metingen waarop het afgeleid model gebaseerd is)
- `nBomenInterval`: aantal metingen waarop het model gebaseerd is (ingeval van basismodel of lokaal model)
- `nBomenOmtrek05`: aantal metingen > 0.5 m binnen het interval (dus het aantal metingen waarop de RMSE-berekening van het basismodel en lokaal model gebaseerd is)
...
Een model is altijd een benadering van werkelijkheid, dat er liefst zo goed mogelijk bij aansluit (cfr. George Box: "All models are wrong, but some are useful"). <!-- spell-check: ignore -->
...
Voor het berekenen van het afgeleid model (= verschuiving van het Vlaams model) worden de lagere omtrekklassen niet meegenomen. Bij de berekening van de foutenmarge (RMSE) worden deze omtrekklassen ook niet meegenomen. Hier bekom je immers het meest optimale model voor je scope als je de Vlaamse curve verschuift zodat deze zo dicht mogelijk aansluit bij de metingen van het gekozen interval.  Bij het meenemen van lagere omtrekklassen bepalen deze metingen mee de ligging van de curve, met een (mogelijk) minder goede benadering in het gekozen interval tot gevolg.
...
Bij het afgeleide model, dat een verschuiving is van het Vlaamse model, vormen deze randpunten geen probleem, dus hier worden alle metingen met omtrek > 0.5 m meegenomen.
...
- domeinen met minstens 10 metingen in de omtrekklassen tussen 0,5 en 2,3 m
...
- model toepassen op de records van het 6de deel, dus de hoogte schatten op basis van de gemeten omtrekklasse en het opgestelde model
